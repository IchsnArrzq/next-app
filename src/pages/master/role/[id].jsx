...
export default function RoleEdit({ permissions, errors }) {
  ...
  const Submit = async () => {
    setVisible(true)
    try {
      const { data } = await axios.put(`/api/role/${id}`, form.values)
      showNotification({
  ...
  const Find = async () => {
    setVisible(true)
    const { data } = await axios.get(`/api/role/${id}/edit`)
    form.setFieldValue('name', data.name)
    form.setFieldValue('permissions', data.permissions.map(item => String(item.id)))
    setVisible(false)
  }
  if (errors) {
      ...
      <Card p='xl' shadow="sm">
        <Card.Section p='md'>
          <Group position='apart'>
            ...
            <Title order={5}>Edit a Role</Title>
          </Group>
        </Card.Section>
...
RoleEdit.getLayout = page => <AppLayout children={page} />
