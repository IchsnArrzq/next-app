...
export default function CategoryMachineEdit() {
    ...
    const Find = async () => {
        setVisible(true)
        try {
            const { data } = await axios.get(`/api/category-machine/${id}/edit`)
            form.setValues(data)
        } catch (error) {
            showNotification({
    ...
    const Submit = async () => {
        setVisible(true)
        try {
            const { data } = await axios.put(`/api/category-machine/${id}`, form.values)
            showNotification({
            ...
            <Card p='xl' shadow="sm">
                <Card.Section p='md'>
                    <Group position='apart'>
                        <Button variant='filled' onClick={() => router.push('/master/category-machine')}>
                            back
                        </Button>
...
CategoryMachineEdit.getLayout = page => <AppLayout children={page} />
