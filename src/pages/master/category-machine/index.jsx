...
export default function CategoryMachineIndex({ categoryMachines, errors }) {
  ...
  const Delete = async (id, page) => {
    ...
    try {
      ...
      if (filters.search == '') {
        await axios
          .get(`/api/category-machine?page=${page}`)
          .then(({ data }) => {
            setPagination({...data})
  ...
  useEffect(() => {
    ...
    setRows(categoryMachines.data.map(value => getRows(value)))
  }, [])
  return (
      ...
      <Card px="xl" py="xl" shadow="sm">
        <Card.Section p="md">
          <Group position="apart">
            ...
            <Button
              variant="filled"
              onClick={() => router.push('/master/category-machine/create')}>
              create
            </Button>
          </Group>
        </Card.Section>
