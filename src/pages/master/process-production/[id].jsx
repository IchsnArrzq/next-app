...
export default function ProcessProductionEdit() {
    ...
    const Find = async () => {
        setVisible(true)
        try {
            const { data } = await axios.get(`/api/process-production/${id}/edit`)
            form.setValues(data)
        } catch (error) {
            showNotification({
    ...
    const Submit = async () => {
        setVisible(true)
        try {
            const { data } = await axios.put(`/api/process-production/${id}`, form.values)
            showNotification({
            ...
            <Card p='xl' shadow="sm">
                <Card.Section p='md'>
                    <Group position='apart'>
                        <Button variant='filled' onClick={() => router.push('/master/process-production')}>
                            back
                        </Button>
...
ProcessProductionEdit.getLayout = page => <AppLayout children={page} />
