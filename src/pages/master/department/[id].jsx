...
export default function DepartmentEdit() {
    ...
    const Find = async () => {
        setVisible(true)
        try {
            const { data } = await axios.get(`/api/department/${id}/edit`)
            form.setValues(data)
        } catch (error) {
            showNotification({
    ...
    const Submit = async () => {
        setVisible(true)
        try {
            const { data } = await axios.put(`/api/department/${id}`, form.values)
            showNotification({
            ...
            <Card p='xl' shadow="sm">
                <Card.Section p='md'>
                    <Group position='apart'>
                        <Button variant='filled' onClick={() => router.push('/master/department')}>
                            back
                        </Button>
...
DepartmentEdit.getLayout = page => <AppLayout children={page} />
