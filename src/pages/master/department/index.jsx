...
export default function DepartmentIndex({ departments, errors }) {
  ...
  const Delete = async (id, page) => {
    ...
    try {
      ...
      if (filters.search == '') {
        axios.get(`/api/department?page=${page}`).then(({ data }) => {
          setPagination({
            ...data,
          })
  ...
  useEffect(() => {
    ...
    setRows(departments.data.map(value => getRows(value)))
  }, [])
  return (
      ...
      <Card px="xl" py="xl" shadow="sm">
        <Card.Section p="md">
          <Group position="apart">
            ...
            <Button
              variant="filled"
              onClick={() => router.push('/master/department/create')}>
              create
            </Button>
          </Group>
        </Card.Section>
...
export async function getServerSideProps(context) {
  try {
    const { data } = await axios.get('/api/department?page=1', {
      headers: {
